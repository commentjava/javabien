...
let _execute lexbuf verbose doPrintAST doPrintTypeEnv skip_type_checking skip_eval debug =
  try
    ...
    if not skip_eval then execute_program ast debug (* TODO: change that to typed_ast *)
  with
    | Error ->
...
let custom_exec lexbuf verbose doPrintAST skip_type_checking skip_eval debug =
  _execute lexbuf verbose doPrintAST false skip_type_checking skip_eval debug
